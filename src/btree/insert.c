...
/*
 * Build a linked list of insert stack items and insert them using
 * o_btree_insert_item(), which can process multiple items sequentially.
 * Each tuple gets its own page-find context.
 */
void
o_btree_insert_items_list(BTreeDescr *desc,
						  OTuple *tuples, LocationIndex *tuplens,
						  BTreeLeafTuphdr *leaf_headers, int ntuples,
						  CommitSeqNo csn, int reserve_kind)
{
	OBTreeFindPageContext *contexts;
	BTreeInsertStackItem *items;
	MemoryContext prev_context;
	bool		nested_call;
	int			i;
	...
	contexts = (OBTreeFindPageContext *)
		palloc(sizeof(OBTreeFindPageContext) * ntuples);
	items = (BTreeInsertStackItem *)
		palloc(sizeof(BTreeInsertStackItem) * ntuples);

	/* Initial reservation required by o_btree_insert_item() */
	ppool_reserve_pages(desc->ppool, reserve_kind, 2);

	for (i = 0; i < ntuples; i++)
	{
		init_page_find_context(&contexts[i], desc, csn,
							   BTREE_PAGE_FIND_MODIFY | BTREE_PAGE_FIND_FIX_LEAF_SPLIT);
		contexts[i].insertTuple = tuples[i];
		contexts[i].insertXactInfo = leaf_headers[i].xactInfo;

		find_page(&contexts[i], &tuples[i], BTreeKeyLeafTuple, 0);

		items[i].next = (i + 1 < ntuples) ? &items[i + 1] : NULL;
		items[i].context = &contexts[i];
		items[i].tuple = tuples[i];
		items[i].tuplen = tuplens[i];
		items[i].tupheader = (Pointer) &leaf_headers[i];
		items[i].level = 0;
		items[i].replace = false;
		items[i].rightBlkno = OInvalidInMemoryBlkno;
		items[i].refind = false;
	}

	o_btree_insert_item(&items[0], reserve_kind);
