...
class NotSupportedYetTest(BaseTest):
	...
	def test_reindex_concurrently(self):
		...
		# Reindex concurrently for orioledb tables
		_, _, err = node.psql("""
			SET SESSION search_path = 'ddl';
			REINDEX (VERBOSE) TABLE CONCURRENTLY o_test_1;
		""")
		self.assertNotIn("not supported for orioledb tables yet", err.decode("utf-8"))

		# Reindex concurrently for orioledb indices
		_, _, err = node.psql("""
			SET SESSION search_path = 'ddl';
		...
		# Reindex concurrently for schema containing orioledb tables
		_, _, err = node.psql("""
			REINDEX (VERBOSE) SCHEMA CONCURRENTLY ddl;
		""")
		self.assertNotIn("not supported for orioledb tables yet", err.decode("utf-8"))

		# Reindex concurrently for database containing orioledb tables
		_, _, err = node.psql("""
