...
8.  `CREATE INDEX CONCURRENTLY` is not supported. `REINDEX CONCURRENTLY` is not supported (falls back to regular `REINDEX` with a warning, or raises an error when `orioledb.strict_mode` is `on`). See [concurrent index creation](concurrent-index.mdx) for details.
...
See [description of OrioleDB's settings](configuration.mdx), [concurrent index creation](concurrent-index.mdx), [experimental decoupled storage and compute mode](decoupled-storage.mdx), and [experimental undo-based rewind](rewind.mdx)
