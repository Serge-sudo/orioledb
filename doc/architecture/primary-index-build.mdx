...
3. **Heap (primary tree) scan and spooling**
   ...
   - Parallel path: `rebuild_indices_worker_sort()` performs the same scan in each
     ...
     shared `oIdxShared`.
   - Page residency rationale: the sequential scan is expected to pull pages into the
     shared buffer pool so later batches (workers or leader) can reuse them without
     repeated disk reads. If pages are not cached, tuplesort accumulation becomes
     I/O-bound; ensuring scan uses shared buffers (not direct/local buffers) is key to
     keep heap-to-sort throughput stable. For buffer manager mechanics (separate from
     WAL/undo buffering), see `doc/architecture/buffering.mdx`.
...
5. **Page construction**
   ...
   - Strategy of `btree_write_index_data()`:
     1. Read tuples from tuplesort in key order; form leaf pages until they hit
        fillfactor, splitting with `stack_page_split()` as needed.
     2. Maintain an in-memory stack of partially built pages at each level; when a
        leaf splits, a downlink (high-key + right-link) is pushed upward via
        `put_downlink_to_stack()`.
     3. Internal pages are filled the same way; splits bubble up until a new root is
        produced. Right-links and high-keys are assigned as pages are sealed.
     4. Completed pages are written sequentially with `btree_page_reorg()` ensuring
        compact layout and flags; the metapage is emitted last with root info and
        checkpoint number.
   - Practical tuning levers and typical bottlenecks:
     - **Fill factor & tuple size spread**: higher fillfactor reduces file size but
       makes `stack_page_split()` happen more often; uneven tuple sizes can cause
       repeated split/reorg work.
     - **Sort output locality**: well-ordered tuples minimize `page_locator` walks and
       reduce memmove during `btree_page_reorg()`.
     - **Hot spots in builder**: page zeroing and tuple copies inside
       `put_item_to_stack()`/`stack_page_split()` dominate CPU; the max-key recalculation
       (`btree_page_update_max_key_len()`) and high-key generation can be sensitive to
       wide keys.
     - **I/O pattern**: `btree_write_index_data()` writes pages sequentially; ensuring
       enough `work_mem`/`maintenance_work_mem` for sorting reduces backpressure and
       keeps the write path streaming.
